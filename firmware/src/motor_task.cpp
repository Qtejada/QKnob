/****************
*
* Runs the SimpleFOC motor control loop and provides haptic feedback through the motor.
* This needs to run at a high speed to ensure a responsive/stable control system, 
* so it’s the only task assigned to core 1.
*
******************/
...
///Calibration Constants. Need to be tuned to my specific motor
...
/// Creates Motor task with predefined parameters of stack depth, ID, etc, along with creating a command queue
...
///initializes magnetic encoderdepending on which sensor we are using
...
void MotorTask::run() {
    ...
    ///set up BUS communication dependent on magnetic sensor
    #if SENSOR_TLV
    ...
    // link the appropriate driver?
    motor.linkDriver(&driver);


    ///parameters for the motor
    ///Controlling Torque in this case
    ...
    ///update encoder magnetic sensor
    encoder.update();
    delay(10);
    ...
    ///Starting postion setup for detents
    ...
    while (1) {
        ///runs FOC loop to get motor angle and speed
        motor.loopFOC();

        /// Check queue for pending requests from other tasks
        Command command;

        ///if command recieved do it
        if (xQueueReceive(queue_, &command, 0) == pdTRUE) {
            switch (command.command_type) {
                ...
                case CommandType::CONFIG: {
                    ...
                    // Update Position from new config
                    ...
                    if (new_config.position != config.position
                        ...
                        ///Position was updated, keep track of that
                        position_updated = true;
                    }

                    //move and adjust to new Min and Max position given by new config
                    ...
                    ///If position was updated, or the radians was changedm we have to update detent center?
                ...
                case CommandType::HAPTIC: {
                    ...
                    for (uint8_t i = 0; i < 3; i++) {
                        ///run through the loop 3 times for some reason
                        motor.loopFOC();
                        delay(1);
                    }
                    motor.move(-strength);
        ...
        if (fabsf(motor.shaft_velocity) > 60) {
            ...
            ///I think this calculates how much torque should be applied
...
///calibrate motor based on FOC library
