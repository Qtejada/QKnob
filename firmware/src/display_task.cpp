...
void DisplayTask::showMessage(const char* message, uint16_t color) {
    spr_.fillSprite(TFT_BLACK);
    spr_.setTextColor(color);
    spr_.setTextDatum(CC_DATUM);
    spr_.drawString(message, TFT_WIDTH / 2, TFT_HEIGHT / 2);
    spr_.pushSprite(0, 0);
}
...
void DisplayTask::run() {
    ...
    ///Invert display colors for some reason
    ...
    const uint16_t FILL_COLOR = spr_.color565(128, 0, 128);  // Purple color
    const uint16_t DOT_COLOR = spr_.color565(186, 85, 211);  // Lighter purple color
